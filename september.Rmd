...
rm(list = ls())
setwd("C:/Users/avery/OneDrive/Desktop/Material Demand")
...
#IRA inputs (includes both cap and cap_y)
...
#ref inputs (includes both cap and cap_y)
...
test <- subset(capacities_all, technology %in% "Coal" & scenario %in% "IRA")
test1 <-test %>% group_by(scenario,year) %>% summarise(newval = mean(value), max = max(value),min = min(value)) #averaging the different model results 
...
#capacities["technology"][capacities["technology"] == "Solar Distributed"] <- "utility-scale solar pv"
...
# Filter rows where year is not equal to 2021
#temp_df <- capacities_y %>%
 # filter(year != 2025)
...
# Step 2: Create a new dataframe with the duplicated rows

# Modify the "year" column for every 3 rows to 2022
...
final_y_cap_IRA["technology"][final_y_cap_IRA["technology"] == "Solar Utility"] <- "utility-scale solar pv"
...
intensity_mean <- subset(intensity_mean, technology %in% repeat_technologies)
...
#materials_demand <- materials_demand %>%
 # select(-max.x, -min.x, -max.y, -min.y, -min, -max) this just makes it look nicer lol
...
```
```{r}
#multiplying material itnesntiy (10,000 itereations) by delta capacity to get new material demand per year. 
n = 1
df_mc <- data.frame() 
...
#result is mc_material_demand is new material inputs needed per year. 
