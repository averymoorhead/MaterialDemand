...
input_wang = read_excel("Combined Intensity Values.xlsx", sheet = "Sheet1", range = cell_cols(1:3))
...
input_wang = unique(input_wang)


#Define the relevant materials 
materials = c("Al", "Cement", "Steel", "Cu", "Si", "Glass", "In", "Nd", "Ag", "Dy", "Ga","Se", "Ni","Mn")

#Define the REPEAT scenarios to be included
scenarios = c("Ref", "IRA")

#Define the REPEAT technologies to be included
repeat_technologies = c("onshore wind", "utility-scale solar pv", "Nuclear New")
...
intensity = unique(intensity) #filtered out duplicates here - I'm not sure if this makes complete sense 
...
materials_demand_sum <-materials_demand %>% group_by(year,scenario) %>% summarise(Al = sum(Al), Cement = sum(Cement), Steel = sum(Steel), Copper = sum(Cu), Silicon = sum(Si), Glass =sum(Glass), Indium = sum(In), Neodymium = sum(Nd), Silver= sum(Ag), Dysprosium  = sum(Dy), Gallium = sum(Ga), Selenium= sum(Se), Nickel = sum(Ni), Magenese= sum(Mn))
...
#summing and summarising all mateiral demand/technology combos 
repeat_materials_demand_sum <-repeat_materials_demand %>% group_by(year,scenario) %>% summarise(Al = sum(Al), Cement = sum(Cement), Steel = sum(Steel), Copper = sum(Cu), Silicon = sum(Si), Glass =sum(Glass), Indium = sum(In), Neodymium = sum(Nd), Silver= sum(Ag), Dysprosium  = sum(Dy), Gallium = sum(Ga), Selenium= sum(Se), Nickel = sum(Ni), Magenese= sum(Mn))
