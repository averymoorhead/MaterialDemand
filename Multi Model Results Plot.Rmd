...
test <- all_models %>% subset(year %in% 2035 & scenario %in% "IRA") %>% select(year,technology, Aluminum_mean, model) 
```
...
sum_material_columns <- function(data, material) {
  material_mean_col <- paste0(material, "_mean")
  material_min_col <- paste0(material, "_min")
  material_max_col <- paste0(material, "_max")
  
  material_demand_sum_df <- data %>%
    select(year, scenario, technology, model, 
           matches(material_mean_col), 
           matches(material_max_col), 
           matches(material_min_col)) %>%
    group_by(year, scenario, model) %>%
    summarise(
      !!material_mean_col := sum(!!sym(material_mean_col), na.rm = TRUE),
      .groups = 'drop'
    ) %>%
    group_by(year, scenario) %>%
    summarise(
      !!material_min_col := min(!!sym(material_mean_col), na.rm = TRUE),
      !!material_max_col := max(!!sym(material_mean_col), na.rm = TRUE),
      .groups = 'drop'
    )
