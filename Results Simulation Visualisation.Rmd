...
melted_transmission_material_sum <- transmission_material_sum %>%
  pivot_longer(cols = c(Cu, Concrete, Steel, Aluminum),
               names_to = "material",
               values_to = "Transmission_mean")
...
combined_plot <- p1 +
  geom_boxplot(data = aluminum_subset, aes(x = year - stagger_width, y = total_demand), fill = "red", alpha = 0.5, width = 0.4) +
