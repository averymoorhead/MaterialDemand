...
```
```{r}
#test = subset(material_intensity, technology == "Wind Offshore" & Material == "Silver")
#fw <- fitdist(test$value, "gamma")
#summary(fw)
...
for (combo in combinations) {
  ...
  # Fit distributions
  fits <- list()
  distributions <- c("weibull", "gamma", "exp","triang","norm")
...
greater_than_30_count <- material_intensity %>%
  inner_join(count_table, by = c("technology", "Material")) %>%
  filter(count < 30) %>%
...
final_aic_results <- bind_rows(greater_than_30_count, final_aic_results)
