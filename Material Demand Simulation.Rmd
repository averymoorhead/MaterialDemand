...
# Loop through each row of ccs_df and add the values to the corresponding 'Gas', 'Coal', or 'utility-scale solar pv' rows
for (i in seq_len(nrow(ccs_df))) {
  ...
  # Add the values from the 'Gas CCS', 'Coal CCS', 'Solar Distributed', or 'NGGT' row to the corresponding row
  if (length(target_row_idx) == 1) {
    materials_demand[target_row_idx, 4:93] <- materials_demand[target_row_idx, 4:93] + row[4:93]
  }
}

# Remove the redundant rows after merging
