...
repeat_technologies = c("onshore wind","utility-scale solar pv", "Nuclear New", "offshore wind","Coal","Geothermal","Biomass","Gas", "Hydro", "Coal CCS", "Gas CCS")
...
# Assuming materials_demand is your dataframe

library(dplyr)

# Assuming materials_demand is your dataframe

# Step 1: Filter the dataframe for rows where technology is 'Gas CCS' or 'Coal CCS'
ccs_df <- materials_demand %>% filter(technology %in% c("Gas CCS", "Coal CCS"))

# Step 2: Loop through each row of ccs_df and add the values to the corresponding 'Gas' or 'Coal' rows
for (i in seq_len(nrow(ccs_df))) {
  ...
  # Determine the corresponding technology
  target_technology <- ifelse(technology == "Gas CCS", "Gas", "Coal")

  # Find the corresponding 'Gas' or 'Coal' row
  ...
  # Add the values from the 'Gas CCS' or 'Coal CCS' row to the 'Gas' or 'Coal' row
...
print(materials_demand)
materials_demand <- materials_demand %>% filter(!technology %in% c("Gas CCS", "Coal CCS"))
  ...
  for (tech in repeat_technologies) {
    ...
    if (nrow(loc) > 1) {
      ...
      if (distribution_info == "triangular") {
        ...
        result <- rnorm(10000, mean = meanv, sd = (maxv - minv) / 6)
      } else if (distribution_info == "weibull") {
        result <- rweibull(10000, shape = 1.5, scale = meanv)
      } else if (distribution_info == "gamma") {
        result <- rgamma(10000, shape = 1.5, scale = meanv / 1.5)
      } else {
...
mc_material_demand_total <- mc_material_demand_total %>%
  mutate(
    total_demand = rowSums(select(., 
                                   c(Coal, 'Coal CCS', 'Gas CCS', Gas, Hydro, `Nuclear New`, `onshore wind`, `utility-scale solar pv`)))
  )
subset_data <- mc_material_demand_total %>%
  filter(Material == "Steel", scenario == "IRA", year == 2035)
