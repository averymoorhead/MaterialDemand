...
repeat_technologies = c("onshore wind","utility-scale solar pv", "Nuclear New", "Wind Offshore","Coal","Geothermal","Biomass","Gas", "Hydro")
...
#  summarise(Aluminum = sum(Aluminum), Cement = sum(Cement), Steel = sum(Steel), Copper = sum(Cu), Silicon = sum(Si), Glass = sum(Glass), Indium = sum(In), Neodymium = sum(Nd), Silver = sum(Ag), Dysprosium = sum(Dy), Gallium = sum(Ga), Selenium = sum(Se), Nickel = sum(Ni), Manganese = sum(Mn))
...
#          -min.x.x, -min.x.x.x, -min.x.x.x.x, -min.x.x.x.x.x, 
#          -max.x, -max.x.x, -max.x.x.x, -max.x.x.x.x, -max.x.x.x.x.x, -max.x.x.x.x.x.x,
#          -min.x.x.x.x.x.x,
#          # Additional removals for columns ending with ".y"
#          -max.y, -min.y, -max.y.y, -min.y.y, -max.y.y.y, -min.y.y.y, -max.y.y.y.y, -min.y.y.y.y, -max.y.y.y.y.y, -min.y.y.y.y.y,-max.y.y.y.y.y.y,-min.y.y.y.y.y.y)
# 
# # Material demand by material, scenario, and year 
# materials_demand_sum <- materials_demand %>% 
#   group_by(year, scenario) %>% 
#   summarise(Aluminum = sum(Aluminum), Cement = sum(Cement), Steel = sum(Steel), Copper = sum(Copper), Silicon = sum(Silicon), Indium = sum(Indium), Neodymium = sum(Neodymium), Silver = sum(Silver), Dysprosium = sum(Dysprosium), Gallium = sum(Gallium), Selenium = sum(Selenium), Nickel = sum(Nickel), Manganese = sum(Manganese))
# 
# # Prevent negative values in materials_demand_sum
# materials_demand_sum[materials_demand_sum < 0] <- 0
...
# Material demand by material, scenario, and year 
#repeat_materials_demand_sum <- repeat_materials_demand %>% 
  #group_by(year, scenario) %>% 
  #summarise(Aluminum = sum(Aluminum), Cement = sum(Cement), Steel = sum(Steel), Copper = sum(Copper), Silicon = sum(Silicon), Indium = sum(Indium), Neodymium = sum(Neodymium), Silver = sum(Silver), Dysprosium = sum(Dysprosium), Gallium = sum(Gallium), Selenium = sum(Selenium), Nickel = sum(Nickel), Manganese = sum(Manganese))

# Prevent negative values in repeat_materials_demand_sum
#repeat_materials_demand_sum[repeat_materials_demand_sum < 0] <- 0
...
# Define repeat technologies and other data frames as needed
subset_capacities <- subset(capacities_new, year == 2025 & scenario == "IRA")

# Process df_mc data
melted_df_mc <- df_mc %>%
  mutate(row_id = row_number()) %>%
  pivot_longer(cols = -row_id, names_to = "Material_Technology", values_to = "Distribution_Value") %>%
...
melted_df_mc <- melted_df_mc %>%
 ...
 merged_data <- merge(melted_df_mc, subset_capacities, by = "technology")

# Print unique technology names
print(unique(subset_capacities$technology))
print(unique(melted_df_mc$technology))

merged_data$demand_2025 = merged_data$Distribution_Value *merged_data$newval
...
#   mutate(row_id = row_number()) %>%
#   pivot_longer(cols = -row_id, names_to = "Material_Technology", values_to = "Distribution_Value")
# merged_data <- merge(melted_df_mc, subset_capacities, by = NULL)
# 
# # Step 3: Multiply distribution fitting values by capacity values
# merged_data <- merged_data %>%
#   mutate(final_value = Distribution_Value * newval)  # Replace capacity_column_name with the actual column name in capacities_new

# Extract Material and Technology from the combined column
# Separate Material and Technology from the combined column
melted_df_mc <- melted_df_mc %>%
  separate(Material_Technology, into = c("Material", "Technology"), sep = "\\.", remove = FALSE, extra = "merge")
...
# capacities_new <- subset(capacities_new, technology %in% repeat_technologies)
# capacities_new <- subset(capacities_new, year %in% c(2025, 2030, 2035)) # Subset for specific years
# names(capacities_new)[names(capacities_new) == "run.name"] <- "scenario"
# capacities_new$scenario[capacities_new$scenario == 'ira_mid'] <- 'IRA'
# capacities_new$scenario[capacities_new$scenario == 'baseline'] <- 'REF'
...
#             print("mc loc below")
#             print(mc_loc$Material)
#             print(mc_loc$technology)
#             print("-")
